...
parser.add_argument('--route_loss_rate', default=2.0, type=float)

##### Modified #####
## A function for decay and normalize reward of Policy Gradients

### decay_normalize_loss(actions_len_all_turn, vt_all_turn, args.scene2action_gamma,args)

def decay_normalize_loss(actions_len_all_turn, vt_all_turn, gamma,args):
  ### 1. turn the vqa loss of the current batch  to [0,1]
  
  b_s = len(vt_all_turn)
  max_e = max(vt_all_turn)
  min_e = min(vt_all_turn)
  dist_mm = max_e - min_e
  if max_e == min_e:
    for i in range(b_s):
      vt_all_turn[i] = 1.0
    ...
    for i in range(b_s):
      vt_all_turn[i] = (vt_all_turn[i] - min_e) / (float)(dist_mm)


  ### 2. copy vqa loss of each turn to turn length;
  ### 3. apply an exponential function 
  return_rewards = torch.zeros(b_s,args.scene2action_action_dim)
  for i in range(b_s):
    temp = torch.zeros(1,args.scene2action_action_dim)
    for j in range(actions_len_all_turn[i]):
      temp[0,j] = vt_all_turn[i]*pow((j+1)/(float(actions_len_all_turn[i])),2)
    

    ##### apply gamma decay
    discounted_rewards = torch.zeros(1,args.scene2action_action_dim)
    running_add = 0

    for t in reversed(range(0, args.scene2action_action_dim)):
      running_add = running_add*gamma + temp[0,t]
      print("t " + str(t))
      discounted_rewards[0,t] = running_add
 
    return_rewards[i,:] = discounted_rewards 

  return return_rewards


"""
def decay_normalize_loss(Minus_reward_currentturn, gamma, args):
  ...
  temp_length = len(discounted_rewards)
  for t in range(len(discounted_rewards)):
    discounted_rewards[t] = discounted_rewards[t] + temp_length/(float)(args.maximum_action_number)
  ...
  while t < args.num_iterations:
    ...
    for batch in train_loader:
      ...
      for turn in range(feats_var.size(0)):
        ...
        #----------------------------------------------------------------------------------------#
        while len(Logits_currentturn) < args.scene2action_action_dim:
          temp = torch.zeros(1,1)
          Logits_currentturn = torch.cat((Logits_currentturn, temp), 0) ##### To check #####

        feats_var[turn] = current_scene

        ##### record loss for current turn #####
        ...
        vt_all_turn.append(loss_current)
        #----------------------------------------------------------------------------------------#

        if turn == 0:
          loss_all_turn = Logits_currentturn.view(1,-1)
        else:
          
          loss_all_turn = torch.cat((loss_all_turn, Logits_currentturn.view(1,-1)), 0)
        actions_len_all_turn.append(len(actions))
      ...
      #loss_all_turn = loss_all_turn.cuda()
      vt = decay_normalize_loss(actions_len_all_turn, vt_all_turn, args.scene2action_gamma,args)
      vt = vt.cuda()
      ...
      loss_current_s2a = torch.mean(loss_all_turn*vt)
      #loss_current_s2a.requires_grad=True
      loss_current_s2a = loss_current_s2a.cuda()

      #loss_current_s2a = torch.mean(loss_all_turn_tensor*vt_tensor) 
